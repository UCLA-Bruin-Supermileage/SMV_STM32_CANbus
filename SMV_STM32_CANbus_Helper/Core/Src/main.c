...
int main(void)
{
  ...
  can1.init(&can1, FC, &hcan1); // methods require pointers to self because C has no "this" pointer
  ...
  while (1)
  {
	   ...
	   if ((HAL_GetTick() - timer)> 1000){
		  timer = HAL_GetTick();
		  can1.send(&can1, 23.2, FC_Pressure);
		  can1.send(&can1, 19.1, Gas);
		  can1.send(&can1, 0.2, Brake);
	  }
...

{
    ...
    if (HAL_CAN_GetRxMessage(CanHandle, CAN_RX_FIFO0, &(can1.RxHeaderFIFO0), can1.RxDataFIFO0) != HAL_OK)
    {
      ...
      if (can1.getHardwareRaw(&can1) == UI && can1.getDataTypeRaw(&can1) == Horn){
			can_read [0] = can1.getData(&can1);
      }else if (can1.getHardwareRaw(&can1) == UI && can1.getDataTypeRaw(&can1) == Wipers){
